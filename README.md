...
| **多戦略比較** | Kelly / 整数計画法 / ベイズ最適化 を同時実行して戦略ごとの損益を比較 |
...
自動モニタリングでは以下 3 戦略を **同時に実行** し、戦略ごとの損益を比較できる。

| 戦略 | 説明 | 特徴 |
|------|------|------|
| **kelly** | 1/4 Kelly 規準 | 上位10予想 × EV > 閾値 の組にケリー配分（**デフォルト**） |
| **ip** | 整数計画法（EV 最大化） | EV 比例配分 → scipy LP 最適化。上限 max_combos 通り |
| **bayes** | ベイズ最適化パラメータ | Optuna で過去データから Kelly パラメータを自動調整 |
...
```
  ...
  --------  ------  --------  --------  --------  -------  -----
  bayes          8     6,400     8,250    +1,850   128.9%   1(13%)
